...
// === SELECTORES ===
const productContainer = document.querySelector('#contenedor-ofertas');
const paginationContainer = document.querySelector('#pagination-nav-ofertas');
...
function displayProducts(page) {
  ...
  paginatedProducts.forEach(producto => {
    ...
    productCard.className = 'flex h-full flex-1 flex-col gap-4 rounded-lg min-w-40';
    ...
    const precioOfertasFormateado = formatearMoneda(producto.Precio_oferta);
    ...
    productCard.innerHTML = `
      ...
      <div>
        <p class="text-[#1b140e] text-base font-medium leading-normal">${producto.Producto}</p>
        <small>${producto.Código} | ${producto.Categoría}</small>
        <p>${producto.Descripción}</p>
        <p class="text-[#97734e] text-sm font-normal leading-normal">Precio público ${precioPublicoFormateado} | Precio Oferta! ${precioOfertasFormateado}</p>
        <p class="text-[#47a6b0] text-sm font-normal leading-normal">${producto.Estado}</p>
      </div>
    `;
...
fetch("./json/productos_ofertas.json")
